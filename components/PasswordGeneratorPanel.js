...
function estimateCrackTime(entropy) {
  ...
  const guessesPerSecond = 1e10; // Modern GPU cracking rate
...
function generateRandomPassword({ length, uppercase, numbers, symbols }) {
  const charset = getCharset({ uppercase, numbers, symbols });
  let password = '';
...
function generateRandomPassphrase(wordCount = 4) {
  // Use a small static list here, replace with full EFF list in real build
...
export default function PasswordGeneratorPanel() {
  ...
  // Auto update strength and entropy
  useEffect(() => {
    let ent = 0;
    if (input && mode === 'password') ent = calculateEntropy(input, options);
    else if (input && mode === 'passphrase') ent = input.length * Math.log2(14); // static sample wordlist
  ...
  function handlePreset(preset) {
    ...
    setTimeout(handleGenerate, 100); // triggers password refresh
  }
  ...
  return (
    <section style={{marginTop:'24px', padding:'0 8px'}}>
      <div style={{maxWidth:700, borderRadius:24, margin:'0 auto', background:'rgba(24,28,48,0.96)', boxShadow:'0 8px 48px #282C53cc', padding:'32px 28px'}}>
        ...
        {/* Top Row: Generator and Strength */}
        ...
        {/* Customization Controls - row for all options */}
